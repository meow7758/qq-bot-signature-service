...
// 根路径
app.get('/', (req, res) => {
  res.json({ service: 'QQ Bot Signature Service', status: 'running' });
});
  ...
  try {
    ...
    if (body.op === 13) {
      ...
      if (!plainToken || !eventTs) {
        ...
        return res.status(400).json({ error: 'Missing plain_token or event_ts' });
      }
      ...
      const message = util.decodeUTF8(String(eventTs) + String(plainToken));
      console.log('  Message to sign:', String(eventTs) + String(plainToken));
...
app.listen(PORT, () => {
  ...
  console.log(`Bot Secret configured: ${QQ_BOT_SECRET ? 'Yes' : 'No'}`);
});
